...
const PhotoGallery = ({albums, photos, getPage, getAuthorPhoto, getDeletePhtoto}) => {
        ...
        React.useLayoutEffect(()=>{
                albums?.map(item => document?.getElementById(`${item?.id}`)?.addEventListener('click', (e)=>{[...e?.target?.classList].includes('del') ? e?.target?.classList.remove('del') : window.outerWidth > 1024 ? e?.target?.classList.add('del') : null}))
        }, [photos, albums, window.outerWidth > 1024])
