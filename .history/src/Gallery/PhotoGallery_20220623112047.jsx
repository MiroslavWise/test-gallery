...
const PhotoGallery = ({albums, photos, getPage, getAuthorPhoto, getDeletePhtoto}) => {
        ...
        console.log('id: ', albums?.map(item => document?.getElementById(`${item?.id}`)?.addEventListener('click', (e)=>{ console.log('click', [...e?.srcElement?.classList].includes('del') ? e?.srcElement?.classList.remove('del') : e?.srcElement?.classList.add('del'))})))
        React.useEffect(()=>{
                console.log('id: ', albums?.map(item => document?.getElementById(`${item?.id}`)?.addEventListener('click', (e)=>{ console.log('click', [...e?.srcElement?.classList].includes('del') ? e?.srcElement?.classList.remove('del') : e?.srcElement?.classList.add('del'))})))
        }, [albums])
        document.addEventListener('keydown', (event)=>{
                if(event.code === 'Delete' || event.code === 'Backspace'){
                        [...document.getElementsByClassName('del')].forEach(item => console.log(item.id))
                }
        })
