...
const PhotoGallery = ({albums, photos, getPage, getAuthorPhoto, getDeletePhtoto}) => {
        ...
        React.useLayoutEffect(()=>{
                albums?.map(item => document?.getElementById(`${item?.id}`)?.addEventListener('click', (e)=>{window.outerWidth > 1024 && ([...e?.target?.classList].includes('del') > 1024 ? e?.target?.classList.remove('del') : e?.target?.classList.add('del'))}))
        }, [photos, albums])
